...
class SalesInvoice(SellingController):
	...
	def update_time_sheet(self, sales_invoice_opd):
		for d in self.timesheets:
			if d.time_sheet:
				timesheet = frappe.get_doc("Timesheet", d.time_sheet)
				self.update_time_sheet_detail(timesheet, d, sales_invoice_opd)
	...
	def update_time_sheet_detail(self, timesheet, args, sales_invoice_opd):
		for data in timesheet.time_logs:
			if (self.project and args.timesheet_detail == data.name) or \
				(not self.project and not data.sales_invoice_opd) or \
				(not sales_invoice_opd and data.sales_invoice_opd == self.name):
				data.sales_invoice_opd = sales_invoice_opd
	...
	def validate_serial_numbers(self):
		...
		self.validate_serial_against_sales_invoice_opd()
	...
	def validate_serial_against_sales_invoice_opd(self):
		...
		for item in self.items:
			...
			for serial_no in item.serial_no.split("\n"):
				sales_invoice_opd = frappe.db.get_value("Serial No", serial_no, "sales_invoice_opd")
				if sales_invoice_opd and self.name != sales_invoice_opd:
					sales_invoice_opd_company = frappe.db.get_value("Sales Invoice", sales_invoice_opd, "company")
					if sales_invoice_opd_company == self.company:
						frappe.throw(_("Serial Number: {0} is already referenced in Sales Invoice: {1}"
							.format(serial_no, sales_invoice_opd)))
	...
	def delete_loyalty_point_entry(self):
		lp_entry = frappe.db.sql("select name from `tabLoyalty Point Entry` where sales_invoice_opd=%s",
			(self.name), as_dict=1)

		if not lp_entry: return
		against_lp_entry = frappe.db.sql('''select name, sales_invoice_opd from `tabLoyalty Point Entry`
			where redeem_against=%s''', (lp_entry[0].name), as_dict=1)
		if against_lp_entry:
			invoice_list = ", ".join([d.sales_invoice_opd for d in against_lp_entry])
			...
			frappe.db.sql('''delete from `tabLoyalty Point Entry` where sales_invoice_opd=%s''', (self.name))
			# Set loyalty program
			self.set_loyalty_program_tier()
	...
	def apply_loyalty_points(self):
		...
		for lp_entry in loyalty_point_entries:
			if lp_entry.sales_invoice_opd == self.name:
				continue
...
def make_delivery_note(source_name, target_doc=None):
	...
	doclist = get_mapped_doc("Sales Invoice", source_name, 	{
		...
		"Sales Invoice Item": {
			...
			"field_map": {
				"name": "si_detail",
				"parent": "against_sales_invoice_opd",
				"serial_no": "serial_no",
